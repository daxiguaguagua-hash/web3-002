...
import { Home, PieChart, Users, Settings, Plus, X, Calendar, CreditCard, ShoppingBasket, Utensils, Car, ShoppingBag, Film, Plane, Zap, Check } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange, onAddClick }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/80 backdrop-blur-xl border-t border-black/5 pb-8 pt-2 px-6">
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1 py-2 px-4 transition-colors ${
                isActive ? 'text-mint' : 'text-muted hover:text-forest'
              }`}
            >
  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 h-[92vh] w-full bg-white rounded-t-[40px] shadow-2xl flex flex-col"
      >
        ...
        <div className="flex flex-col items-center px-6 pt-4 pb-6">
          <div className="flex items-baseline justify-center gap-1 mb-6 w-full">
            <span className="text-3xl text-forest font-display font-medium">$</span>
            <span className="font-mono text-[64px] leading-tight text-ink">{amount}</span>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-3 w-full">
            ...
            <button className="flex items-center gap-2 px-4 py-2.5 bg-canvas rounded-xl hover:bg-slate-100 transition-colors">
              ...
              <span className="text-sm font-medium">Chase Sapphire</span>
            </button>
          </div>
        </div>
              ...
              return (
                ...
                >
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
                    isSelected 
                      ? 'bg-mint text-white shadow-lg ring-4 ring-mint/20 scale-105' 
                      : 'bg-canvas border border-slate-200 text-muted hover:border-mint/50 hover:text-mint'
                  }`}>
                    <Icon size={28} />
                  </div>
                  <span className={`text-xs font-display ${isSelected ? 'font-bold text-forest' : 'font-medium text-muted'}`}>
                    {cat.name}
                  </span>
                </button>
              );
